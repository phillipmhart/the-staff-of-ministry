...
const shell = content => {
  ...
  const progress = /ANSWER ACCEPTED|CHAPTER COMPLETE/.test(shown) ? '<button class="button ghost" onclick="caseMap()">Open case map</button>' : '';
...
function complete2() {
  ...
  app.innerHTML = shell(`<p class="eyebrow">ANSWER ACCEPTED</p><h2 style="text-align:center">Correct! The clue points to Storrs Library.</h2><p>Head toward Storrs Library to continue the adventure.</p>${gallery(fig('c2-storrs-map-illustrated.png', 'Location of Storrs Library'))}<p><strong>You are ready to move onto the next chapter.</strong></p><button class="button" onclick="chapter3Start()">Open Chapter 3</button>`);
  window.scrollTo({ top: 0 });
}
...
function complete6() {
  ...
  app.innerHTML = shell(`<p class="eyebrow">ANSWER ACCEPTED</p><h2 style="text-align:center">Correct!</h2><p>As you head toward the center of town, your phone buzzes with an urgent <strong>text from Mrs. Orac</strong>: “Quick, go to Razzmatazz! A new cryptic note was just spotted outside the entrance!”</p>${gallery(fig('c6-orac-text.jpg', 'Text message from Mrs. Orac'), fig('c7-location-illustrated.png', 'Location of Razzmatazz Kids'))}<p><strong>You are ready to move onto the next chapter.</strong></p><button class="button" onclick="chapter7()">Open Chapter 7</button>`);
  window.scrollTo({ top: 0 });
}
